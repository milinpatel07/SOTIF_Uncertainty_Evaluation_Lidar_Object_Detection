...
correlates with detection correctness, supporting:
...
- **CARLA Synthetic** (SOTIF-PCOD): 547 simulated LiDAR point clouds from
...
is an approximation for methodology demonstration.
...
4. **Gate structure must adapt to the domain.** KITTI's near-perfect
   confidence separation (AUROC=0.999) permits simple confidence gates.
   CARLA's diverse weather requires multi-indicator gates leveraging
   geometric disagreement (AUROC=0.974).

5. **Cross-domain consistency.** The methodology applied to
   CARLA produces uncertainty patterns consistent with KITTI,
   suggesting that simulated data can complement real-world data for
   initial SOTIF evaluation.
...
4. **Physics-based weather augmentation** for LiDAR point clouds
   (rain/fog/snow/spray) for systematic triggering condition
